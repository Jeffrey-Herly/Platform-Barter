...
class BarterTransactionService {
    ...
    /**
     * Cancel barter offer (only by requester, and only if PENDING)
     * @param {string} transaction_id - Transaction ID
     * @param {string} user_id - User yang cancel (must be requester)
     * @returns {Promise<object>} Updated transaction
     */
    async cancelBarterOffer(transaction_id, user_id) {
        try {
            // Get transaction
            const transaction = await this.transactionRepository.findById(transaction_id);
            if (!transaction) {
            ...
            // Check authorization - hanya requester yang bisa cancel
            if (transaction.requester_id !== user_id) {
                throw new Error('Anda tidak memiliki akses untuk membatalkan penawaran ini');
            }

            // Cek apakah masih PENDING
            if (transaction.status_name !== 'PENDING') {
                throw new Error(`Tidak dapat membatalkan penawaran karena status saat ini adalah ${transaction.status_name}`);
            }

            // Get status
            const cancelledStatus = await this.transactionRepository.getStatusByName('CANCELLED');
            if (!cancelledStatus) {
                throw new Error('Status CANCELLED tidak ditemukan');
            }
            ...
            // Create system notification untuk owner bahwa penawaran dibatalkan
            const requesterUser = await this.userRepository.findById(user_id);
            await this.notificationService.createSystemNotification(
                transaction.owner_id,
                'Penawaran Dibatalkan',
                `${requesterUser.full_name} telah membatalkan penawarannya untuk item Anda.`,
                transaction_id,
                'TRANSACTION'
            );
            ...
            return {
                success: true,
                message: 'Penawaran barter berhasil dibatalkan',
                data: {
                    transaction_id: updated.transaction_id,
                    status: 'CANCELLED'
                }
