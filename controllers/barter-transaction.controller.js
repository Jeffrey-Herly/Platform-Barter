...
class BarterTransactionController {
    ...
    /**
     * POST /barter/:transaction_id/cancel
     * Cancel barter offer (by requester)
     */
    async cancelBarterOffer(request, reply) {
        try {
            const { transaction_id } = request.params;
            const user_id = request.user.userId;
            ...
            const result = await this.transactionService.cancelBarterOffer(transaction_id, user_id);

            return reply.send(result);
        } catch (error) {
            console.error('Error in cancelBarterOffer:', error);
            return reply.code(400).send({
                success: false,
                message: error.message || 'Gagal membatalkan penawaran barter'
            });
        }
    }
